...
class CricketClassifier(LightningModule):
  ...
  def configure_optimizers(self):
    ...
    scheduler = {
        'scheduler': OneCycleLR(optimizer, max_lr=1e-5, total_steps=num_training_steps, anneal_strategy='linear', pct_start=warmup_steps/num_training_steps, div_factor=25.0, final_div_factor=10000.0),
        'interval': 'step',
        'frequency': 1,
    }
...
def main(train_data_path,test_data_path,epochs):
  train_set = CustomDataset(datapath = train_data_path, type = 'species')
  ...
  test_set = CustomDataset(datapath = test_data_path, label2id = label2id, id2label = id2label, type = 'species')
  

  lr_monitor = LearningRateMonitor(logging_interval="step")

  # Set up TensorBoard logger
  tb_logger = TensorBoardLogger("log/3_species/try_balanced_ep10", name="BS8_ep10_LR1e-5_balanced_another_trial")
  tb_logger.log_hyperparams({"id2label" : json.dumps(id2label)})

  
  # put the checkpoint to save the model at each epoch  
  checkpoint_callback = ModelCheckpoint(
      dirpath = "/gpfs/proj1/choe_lab/tanu/Species_classification/saved_model_species/3_species/LR1e-5/try_balanced_ep10",
      filename = "{epoch}",
      save_top_k = -1,
...
if __name__ == '__main__':
  ...
  train_data_path = r"/gpfs/proj1/choe_lab/tanu/Species_classification/features_extracted/3_species/balanced_train_750_2273/cricket_data_feature_extracted.pt"
  test_data_path = r"/gpfs/proj1/choe_lab/tanu/Species_classification/features_extracted/3_species/test/cricket_data_feature_extracted.pt"
